...
enum ENUM_LAST_ORDER_DIR
  {
   LAST_ORDER_NONE = 0, // Belum ada order sebelumnya (Bebas)
   LAST_ORDER_BUY  = 1, // Order terakhir adalah BUY
   LAST_ORDER_SELL = -1 // Order terakhir adalah SELL
  };
...
input group "=== 2. Aturan Posisi, Alur & Stacking ==="
input bool               InpAlternateOrders     = true;           // Mode Selang-Seling (Wajib Bergantian: BUY -> SELL -> BUY -> SELL)
...
input string             InpTradeComment        = "EMA9_17_v1.50";// Komentar Order
...
ENUM_LAST_ORDER_DIR g_lastOrderDirection = LAST_ORDER_NONE;

int CloseAllOrders();
ENUM_LAST_ORDER_DIR DetectLastOrderDirection();
...
//+------------------------------------------------------------------+
//| Deteksi Arah Order Terakhir (Dari Posisi Aktif atau Deal History)|
//+------------------------------------------------------------------+
ENUM_LAST_ORDER_DIR DetectLastOrderDirection()
  {
   // 1. Cek dari posisi aktif saat ini (ambil yang paling baru dibuka)
   datetime latestPosTime = 0;
   ulong latestTicket = 0;
   ENUM_LAST_ORDER_DIR posDir = LAST_ORDER_NONE;
         ...
         if(m_position.Symbol() == _Symbol && m_position.Magic() == InpMagicNumber)
           {
            ...
            if(posDir == LAST_ORDER_NONE || t > latestPosTime || (t == latestPosTime && ticket > latestTicket))
              {
               latestPosTime = t;
               latestTicket = ticket;
               posDir = (m_position.PositionType() == POSITION_TYPE_BUY) ? LAST_ORDER_BUY : LAST_ORDER_SELL;
              }
           }
        }
     }

   if(posDir != LAST_ORDER_NONE)
      return posDir;

   // 2. Jika tidak ada posisi aktif, cek dari riwayat transaksi yang sudah tertutup
               ...
               if(entry == DEAL_ENTRY_IN)
                 {
                  ...
                  if(dType == DEAL_TYPE_BUY) return LAST_ORDER_BUY;
                  if(dType == DEAL_TYPE_SELL) return LAST_ORDER_SELL;
                 }
              }
           }
        }
...
int OnInit()
  {
   ...
   // Inisialisasi Deteksi Arah Order Terakhir
   g_lastOrderDirection = DetectLastOrderDirection();
   string lastOrderText = (g_lastOrderDirection == LAST_ORDER_BUY) ? "BUY" : 
                          (g_lastOrderDirection == LAST_ORDER_SELL) ? "SELL" : "BELUM ADA";
   PrintFormat("Mode Selang-Seling: %s | Order Terakhir Terdeteksi: %s", 
               (InpAlternateOrders ? "AKTIF" : "NONAKTIF"), lastOrderText);
...
void CheckSignalAndExecute(ENUM_TIMEFRAMES tf)
  {
   ...
   //--- Validasi Mode Selang-Seling (Wajib Bergantian BUY <-> SELL)
   if(InpAlternateOrders)
     {
      if(buySignal && g_lastOrderDirection == LAST_ORDER_BUY)
        {
         Print("Mode Selang-Seling AKTIF: Order sebelumnya adalah BUY. Sinyal BUY dilewati, menunggu sinyal SELL.");
         return;
        }
      if(sellSignal && g_lastOrderDirection == LAST_ORDER_SELL)
        {
         Print("Mode Selang-Seling AKTIF: Order sebelumnya adalah SELL. Sinyal SELL dilewati, menunggu sinyal BUY.");
         return;
        }
     }
...
void OpenBuy(double shift1High, double shift1Low)
  {
   ...
   if(!m_trade.Buy(lotSize, _Symbol, ask, sl, tp, InpTradeComment))
   ...
   else
     {
      ...
      g_lastOrderDirection = LAST_ORDER_BUY;
     }
  }
...
void OpenSell(double shift1High, double shift1Low)
  {
   ...
   if(!m_trade.Sell(lotSize, _Symbol, bid, sl, tp, InpTradeComment))
   ...
   else
     {
      ...
      g_lastOrderDirection = LAST_ORDER_SELL;
     }
  }
...
void UpdateDashboard(ENUM_TIMEFRAMES tf)
  {
   ...
   string alternateDesc = "DISABLED";
   if(InpAlternateOrders)
     {
      string lastDir = (g_lastOrderDirection == LAST_ORDER_BUY) ? "BUY -> Tunggu SELL" :
                       (g_lastOrderDirection == LAST_ORDER_SELL) ? "SELL -> Tunggu BUY" : "BEBAS (Belum Ada)";
      alternateDesc = StringFormat("ACTIVE (%s)", lastDir);
     }

   string text = "========================================\n" +
                 "  EMA 9 & 17 PIERCING BREAKOUT EA v1.50 (MT5) \n" +
                 " Mode Breakout : " + ((InpBreakoutMode == BREAKOUT_STRICT_1BAR) ? "Strict 1-Bar" : "Flexible 2-Bar") + "\n" +
                 " Selang-Seling : " + alternateDesc + "\n" +
                 "========================================\n" +
                 " Simbol        : " + _Symbol + "\n" +
